...
package body destoc is
   ...
   --Procedimiento que consigue el nodo más a la izquierda, bajando por el hijo
   --derecho del pnodo
   ...
   --Procedimiento que baja por una rama hasta encontrar a partir de qué nodo
   ...
   begin
      if p=null then
         p:= new nodo; p.all:= (null, null, x, k, 0, null, null); -- 0 está igualado
         aux := p;
         h:= true;
      else
         if k<p.k then
            poner(p.lc, k, x, h, aux); --subárbol izq
            if h then balanceo_izq (p, h, insert_mode); end if ;
         elsif k>p.k then
            poner(p.rc, k, x, h, aux); --subárbol der
